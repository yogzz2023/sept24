...
class CVFilter:
    ...
    def initialize_filter_state(self, x, y, z, vx, vy, vz, time):
        if not self.first_rep_flag:
            ...
            self.vx = (self.Z1 - self.Z2) / dt
            self.vy = (self.Z1 - self.Z2) / dt
            self.vz = (self.Z1 - self.Z2) / dt
            self.Meas_Time = time
...
def gating_check(track, measurement, doppler_threshold, range_threshold):
    ...
    last_cartesian = sph2cart(last_measurement, last_measurement, last_measurement)
    measurement_cartesian = sph2cart(measurement, measurement, measurement)
    ...
    for group_idx, group in enumerate(measurement_groups):
        for measurement in group:
            ...
            for track_id, track in enumerate(tracks):
                ...
                distance = np.linalg.norm(np.array(measurement_cartesian) - np.array(last_cartesian))
                time_diff = measurement_time - last_measurement
                ...
                if range_satisfied and time_diff <= time_threshold:
                    if track_id not in firm_ids:
                        if track_id in tentative_ids:
                            ...
                            hit_counts[track_id] = 1
                            miss_counts[track_id] = 0
                            state_map[track_id] = progression_states
            ...
            if not assigned:
                ...
                tracks.append({
                    ...
                    'measurements': [(measurement, progression_states)]
                })
                ...
                state_map[new_track_id] = progression_states
...
def jpda_filter(track, validated_measurements, kalman_filter):
    if validated_measurements:
        ...
        x, y, z = sph2cart(avg_measurement, avg_measurement, avg_measurement)
        kalman_filter.predict_step(avg_measurement)
        Z = np.array([[x], [y], [z]])
        ...
        for track_id, track in enumerate(tracks):
            ...
            if validated_measurements:
                ...
                if current_state == 'Firm':
                    ...
                    r_val, az_val, el_val = cart2sph(kalman_filter.Sf, kalman_filter.Sf, kalman_filter.Sf)
                    t.append(validated_measurements)  # Using the time from the first validated measurement
                    rnge.append(r_val)
                    azme.append(az_val)
                    elem.append(el_val)
